...
function mountVideoInside(host: HTMLElement): HTMLVideoElement {
  ...
  video.muted = true;
  video.autoplay = true;
  // Suppress the iOS Safari/WKWebView ▶ overlay on inline <video>s. iOS shows
  // `-webkit-media-controls-start-playback-button` for any video it considers
  // pausable; the scanner is play-only so we strip controls here and back it up
  // with `::-webkit-media-controls-*` rules in globals.css.
...
/**
 * Centered-square crop rect for one decode region, capped at `canvasCap`.
 *
 * The visible square is the shorter video dimension scaled by `sourceScale`,
 * centered; the destination canvas is that square clamped to `canvasCap` so we
 * never hand ZXing more than ~2K px (anything larger is binned anyway).
 *
 * @internal exported for testing — see backend-zxing-wasm.test.ts
 */
 ...
 * Otherwise — with the WASM module cached from a prior mount — the worker can
 * finish `prepareZXingModule` and post `"ready"` during the camera startup
 * awaits, and a listener attached later will never see it. The decode loop would
 * then hang forever on `await bridge.ready` and the user sees a frozen
 * viewfinder. See `startZxingWasmScanner`.
 */
    ...
    void (async () => {
      try {
        ...
        if (decodedText != null) {
          ...
          // bad/unsupported QR — `onDecoded` does not always navigate away
          // (e.g. a multipart UR fountain emits one fragment per frame, all of
          // which differ, and an unsupported single QR should not freeze the
          // loop). Without this we would re-emit the same payload every frame.
          if (decodedText !== lastPayload) {
            lastPayload = decodedText;
            const singleLine = decodedText.replace(/\s+/g, " ").trim();
            const preview =
              singleLine.length <= 160 ? singleLine : `${singleLine.slice(0, 160)}…`;
            console.info("[t3rminal/scanner/zxing-wasm] decoded QR", {
          ...
          if (timestamp - lastProbeLogAt >= DECODE_PROBE_LOG_INTERVAL_MS) {
            ...
            console.info("[t3rminal/scanner/zxing-wasm] camera frame read; no QR decoded yet", {
        ...
        console.warn("[t3rminal/scanner/zxing-wasm] decode failed", caught);
...
export async function startZxingWasmScanner(
  ...
  const myTurn = startupQueue.catch(() => undefined).then(async () => {
    // Swap out any previous active scan before starting a new one — the module
    // singleton owns exactly one camera + worker at a time.
    if (active != null) {
    ...
    // Attach the message listener BEFORE any async camera setup. With the WASM
    // module cached from a prior mount, the worker can post `"ready"` within the
    // same task as `video.play()`'s await yield — a listener attached after the
    // await would miss the message and the decode loop would hang forever on
    // `bridge.ready`. This is the "scan again doesn't detect" regression: first
    // scan works because the WASM download dominates, second scan races and
    // loses.
    ...
    try {
      ...
      console.info("[t3rminal/scanner/zxing-wasm] camera ready", {
