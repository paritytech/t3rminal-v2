...
const moduleReady = prepareZXingModule({
  ...
  overrides: {
    // Serve the `.wasm` from THIS deployment's origin root, not the default
    // jsDelivr CDN. `scripts/copy-zxing-wasm.mjs` stages it into `public/`, so
    // `output: 'export'` ships it to `/zxing_reader.wasm` — the same origin-root
    // strategy Next uses for its own `/_next/` assets. A host iframe's CSP can
    // block the CDN, and a point-of-sale terminal must not depend on a
    // third-party host to decode a QR. The worker is same-origin with the page,
    // so `self.location.origin` is the deployment origin.
    locateFile(path: string, prefix: string) {
      return path.endsWith(".wasm")
        ? new URL("/zxing_reader.wasm", self.location.origin).href
        : `${prefix}${path}`;
    },
  },
});
